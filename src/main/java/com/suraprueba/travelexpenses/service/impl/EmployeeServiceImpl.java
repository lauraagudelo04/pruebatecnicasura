...
public class EmployeeServiceImpl implements IEmployeeService {
    ...
    public Page<Employee> findAllWithExpensesInDateRange(LocalDate startDate, LocalDate endDate, Pageable pageable) {
        Page<Employee> employeesPage = employeeRepository.findAllWithExpensesInDateRange(startDate, endDate, pageable);

        if (employeesPage.isEmpty()) {
            throw new ResponseStatusException(HttpStatusCode.valueOf(404),
                    "No se encontraron empleados con gastos en el rango de fechas proporcionado.");
        }
